...
$history = 'mobile-nav__item--active';
$page = 'history';
?>
        ...
        <nav class="mobile-nav mobile-nav-top">
            <div class="mobile-nav__item">
                <h4 class="mobile-nav__item-content">
                    Last 7-days Predictions Results
                </h4>		
            </div>
        </nav>
            ...
            <div class="row justify-content-center">	
                ...
                $sql = <<<SQL
                    SELECT `id`,`kickoff`,`home`,`away`,`prediction`,`probability`,`result`, `status`
                            FROM `matches`
                                    WHERE DATE(`kickoff`)>=DATE_SUB(CURDATE(),INTERVAL 7 DAY) AND DATE(`kickoff`)<CURDATE()
                                            AND `posted`=1
...
SQL;
                ...
                if ($stmt = $mysqli->prepare($sql)) {
                    ...
                    $count = $won = 0;
                    while ($stmt->fetch()) {
                        //$status = getStatus($prediction,$result);
                        $count++;
                        $won += ($status == 'LOST' ? 0 : 1);
                        ?>
